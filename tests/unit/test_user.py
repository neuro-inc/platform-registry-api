...
from platform_registry_api.user import InMemoryUserService, User, UserServiceException
...
class TestInMemoryUserService:
    ...
    async def test_get_user_by_name_not_found(self, config):
        service = InMemoryUserService(config=config)
        with pytest.raises(UserServiceException, match='User "unknown" was not found'):
    ...
    async def test_get_user_with_credentials_invalid(self, config):
        ...
        with pytest.raises(UserServiceException, match='User "testuser" was not found'):
