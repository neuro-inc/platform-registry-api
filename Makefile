...
ifdef CIRCLECI
    DEVPI_URL ?= "https://$(DEVPI_USER):$(DEVPI_PASS)@$(DEVPI_HOST)/$(DEVPI_USER)/$(DEVPI_INDEX)"
else
...
build:
	-docker build --build-arg pip_install_opts="$(PIP_INSTALL_OPTS)" -t $(IMAGE_NAME):$(IMAGE_TAG) .
